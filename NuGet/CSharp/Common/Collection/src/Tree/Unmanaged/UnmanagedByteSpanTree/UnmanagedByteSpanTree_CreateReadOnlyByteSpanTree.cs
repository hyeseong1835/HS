    ...
    where TValue : unmanaged
{
    ...
    /// <returns></returns>
    static ReadOnlyByteSpanTree<TValue> CreateBranchTree(byte[] keySlice, UnmanagedLinkedList<ByteNode> byteNodeList, int branchDepth, int valueCount)
    {
        ReadOnlyTreeBuildData buildData = new ();
        buildData.nodeCount = 1;
    ...
    /// <param name="childFindByteIndex"></param>
    /// <returns></returns>
    static ReadOnlyByteSpanTree<TValue> CreateValueBranchTree(byte[] keySlice, TValue value, UnmanagedLinkedList<ByteNode> byteNodeList, int branchDepth, int valueCount)
    {
        ReadOnlyTreeBuildData buildData = new ();
        buildData.nodeCount = 1;
        buildData.valueCount = 1;
    ...
    static ReadOnlyByteSpanTree<TValue>.Node CreateNode(byte[] keySlice, ref UnmanagedByteSpanTree<ByteNode>.ByteNode byteNode, int nodeDepth, int branchDepth, ref ReadOnlyTreeBuildData buildData)
    {
        // 자식이 없음 -> 값 노드
        if (byteNode..IsEmpty)
        {
    ...
    static ReadOnlyByteSpanTree<TValue>.Node[] CreateNodeArray(int parentNodeDepth, int parentBranchDepth, UnmanagedLinkedList<ByteNode> byteNodeList, ref ReadOnlyTreeBuildData buildData)
    {
        ...
        UnmanagedLinkedList<ByteNode>.Enumerator byteNodeEnumerator = byteNodeList.GetEnumerator();
        ...
        {
            if (byteNodeEnumerator.MoveNext())
            {
                ref ByteNode byteNode = ref byteNodeEnumerator.CurrentRef;
    ...
    public ReadOnlyByteSpanTree<TValue> BuildToReadOnlyByteSpanTree()
    {
        ...
        {
            ref ByteNode startNode = ref byteNodeTree.HeadByteNode;
